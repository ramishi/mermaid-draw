...
export function CommandPalette({ onClose }: CommandPaletteProps) {
  ...
  const { undo, redo, setNodes, setDrawingShape, alignSelected, distributeSelected } = useFlowStore(
    useShallow((s) => ({
      undo: s.undo,
      redo: s.redo,
      setNodes: s.setNodes,
  ...
  const handleAutoLayout = async () => {
    const { nodes, edges, direction, layoutEngine, nodeSpacing, rankSpacing } = useFlowStore.getState()
    if (nodes.length === 0) return
    setNodes(await applyLayout(nodes, edges, direction, { layoutEngine, nodeSpacing, rankSpacing }))
    onClose()
  }
  ...
  const actionCommands: CommandItem[] = [
    ...
    { id: 'align-left', label: 'Align Left', description: 'Align selected nodes', icon: '↤', action: () => { alignSelected('left'); onClose() } },
    { id: 'align-center', label: 'Align Center', description: 'Align selected nodes', icon: '↔', action: () => { alignSelected('center'); onClose() } },
    { id: 'align-top', label: 'Align Top', description: 'Align selected nodes', icon: '↥', action: () => { alignSelected('top'); onClose() } },
    { id: 'align-middle', label: 'Align Middle', description: 'Align selected nodes', icon: '↕', action: () => { alignSelected('middle'); onClose() } },
    { id: 'distribute-horizontal', label: 'Distribute Horizontally', description: 'Space selected nodes evenly', icon: '⇔', action: () => { distributeSelected('horizontal'); onClose() } },
    { id: 'distribute-vertical', label: 'Distribute Vertically', description: 'Space selected nodes evenly', icon: '⇕', action: () => { distributeSelected('vertical'); onClose() } },
